...
import org.moeaframework.algorithm.MOGBHS;
import org.moeaframework.algorithm.MOGBHSD;
import org.moeaframework.algorithm.MSOPS;
...
/*
 * This class include the method for building an algorithm based on MOEA/D-DE-ACA 
 * (alias: MOEAD2) or MOEA/D-DE-CA (alias: MOEAD3)
 * Changes in relation to MOEA Framework 2.12: 
 * - Line 284: You have to define the ACAsDirectory attribute (directory with ACA files)
 * - Line 285: You have to define the CAsDirectory attribute (directory with CA files)
 * - Lines 303: The string "MOEAD2" is used to call the new method called newMOEAD2
 * - Lines 305: The string "MOEAD3" is used to call the new method called newMOEAD3
 * - Lines 1398 to 1563: New method to create the MOEAD2 algorithm
 * - Lines 1575 to 1577: New method to create the MOEAD2 (MOEA/D-DE-CA) algorithm 
*/
...
public class StandardAlgorithms extends AlgorithmProvider {
	...
	String ACAsDirectory = "C:\\Users\\cobos\\workspace\\MOEAD_DE_ACA\\resource\\ACAs\\";
	String CAsDirectory = "C:\\Users\\cobos\\workspace\\MOEAD_DE_ACA\\resource\\ACAs\\";
	...
	public Algorithm getAlgorithm(String name, Properties properties,
		...
		try {
			...
			}else if (name.equalsIgnoreCase("MOEAD2")) {
				return newMOEAD2(typedProperties, problem);
			}else if (name.equalsIgnoreCase("MOEAD3")) {
				return newMOEAD3(typedProperties, problem);
			} else if (name.equalsIgnoreCase("MOGBHS")) {
				return newMOGBHS(typedProperties, problem);
			} else if (name.equalsIgnoreCase("MOGBHSD")) {
				return newMOGBHSD(typedProperties, problem);
	...
	private Algorithm newNSGAII(TypedProperties properties, Problem problem) {
		...
		//System.out.println("POBLACION MOGBHS= " + populationSize);
	...
	private Algorithm newMOGBHS(TypedProperties properties, Problem problem) {
		...
		return new MOGBHS(problem, population, null, initialization);
	}
	...
	private Algorithm newMOEAD(TypedProperties properties, Problem problem) {
		...
		//System.out.println("POBLACION MOEAD= " + populationSize);
	...
	private Algorithm newSPEA2(TypedProperties properties, Problem problem) {
		...
		//System.out.println("POBLACION SPEA2 = " + populationSize);
	...
	private Algorithm newMSOPS(TypedProperties properties, Problem problem) {
		...
		//System.out.println("POBLACION MSOPS= " + populationSize);
	...
	private Algorithm newMOGBHSD(TypedProperties properties, Problem problem) {
		if (!checkType(RealVariable.class, problem)) {
			throw new FrameworkException("unsupported decision variable type");
		}
	...
	/**
	 * Returns a new {@link MOEAD2} instance.  Only real encodings are supported.
	 * MOEAD2 = MOEA/D-DE-ACA or MOEA/D-DE-CA depends on property CAType 
	 * 
	 * @param properties the properties for customizing the new {@code MOEAD}
	 *        instance
	 * @param problem the problem
	 ...
	 * @throws FrameworkException if the decision variables are not real valued
	 */	
	private Algorithm newMOEAD2(TypedProperties properties, Problem problem) {
		if (!checkType(RealVariable.class, problem)) {
			throw new FrameworkException("unsupported decision variable type");
		}
		...
		ArrayList<Integer> alphaValues = new ArrayList<Integer>();
				
		String caType = properties.getString("CAType", "ACA");
		String nameCA = properties.getString("nameCA", "N838k100v17^100t2.ca"); // by default the largest one
		String caFile = ACAsDirectory + nameCA;
		
		if (caType == "CA"){
			caFile = CAsDirectory + nameCA;			
		}
			
		FileReader fr;
		try {
			// 1. Load rows from CA or ACA
			fr = new FileReader(caFile);
			BufferedReader bf = new BufferedReader(fr);
			
			ArrayList<int[]> rowsOfTheCA = new ArrayList<int[]>();
			String sline = new String();
			try {
				while ((sline = bf.readLine())!=null) {
					int[] arr = new int[nobj];
			...
			//2. Remove duplicate rows (when remove columns increase the probability of duplicates)
			for (int i = 0; i < rowsOfTheCA.size(); i++){
				int j = i + 1;
				while (j < rowsOfTheCA.size()){
					boolean isEqual = true;
					for (int k = 0; k < nobj; k++){
						if (rowsOfTheCA.get(i)[k] != rowsOfTheCA.get(j)[k]){
							isEqual = false;
							break;
						}
					}
					if (isEqual == true){
						rowsOfTheCA.remove(j);
					}
					else{
						j++;
					}
			...
			//3. Convert cells to float and normalize in the range [0,1] 
			for (int i = 0; i < rowsOfTheCA.size(); i++){
				double sumValues = 0.0;
				int conteo = 0;
				double[] arr2 = new double[nobj];
				for (int k = 0; k < nobj; k++){
					if (rowsOfTheCA.get(i)[k] == 0) conteo++;
					double newValue = rowsOfTheCA.get(i)[k];  // / maximoAlfabeto;
					arr2[k] = newValue;
					sumValues += newValue;
				}
				if (Double.isNaN(sumValues) || Double.isInfinite(sumValues) || sumValues <= 1e-15 || conteo == nobj){
					continue;
				}
			...
			//4. Delete duplicated rows after conversion (less probable)
			for (int i = 0; i < weightRow.size(); i++){
				int j = i + 1;
				while (j < weightRow.size()){
					boolean igual = true;
					for (int k = 0; k < nobj; k++){
						if (Math.abs(weightRow.get(i)[k] - weightRow.get(j)[k]) > 1e-10){
							igual = false;
							break;
						}
					}
		...
		//4. Set the populationSize based on the final number of rows in variable filas
		int populationSize = weightRow.size();	
		properties.setString("populationSize", Integer.toString(populationSize));
		...
		System.out.println("Population MOEAD2 = " + populationSize);
		...
		algorithm.weightRows = weightRow;  // With this property the weights are passed so that they are then initialized

		algorithm.PopulationSize = algorithm.weightRows.size();
		return algorithm;
	}

	/**
	 * Returns a new {@link MOEAD3} instance.  Only real encodings are supported.
	 * MOEAD3 = MOEA/D-DE-CA
	 * 
	 * @param properties the properties for customizing the new {@code MOEAD}
	 *        instance
	 * @param problem the problem
	 * @return a new {@code MOEAD3} instance
	 * @throws FrameworkException if the decision variables are not real valued
	 */	
	private Algorithm newMOEAD3(TypedProperties properties, Problem problem) {
		properties.setString("CAType", "CA");
		return newMOEAD2(properties, problem);
	}
}